...
exports.postMessage = (req, res, next) => {
  if (!req.body.description | !req.body.userID | !req.body.postName) {
    res.status(400).json({ error: "Valeur manquante" });
  } else {
...
exports.likeMessage = (req, res, next) => {
  if (!req.body.userID) {
    ...
    const sqlOne =
      " SELECT * FROM groupomania.likes WHERE id_post = ? AND id_user = ?";
    ...
    db.query(sqlOne, values, (err, result) => {
      ...
      console.log(
        "Like trouver pour l'id " + req.body.userID + " est de " + result.length
      );
      if (result.length === 0) {
        ...
        db.query(sqlCreate, valuesCreate, (err, result) => {
          ...
          if (!err) res.status(200).json({ isLike: true });
        });
        ...
        db.query(sqlDelet, valuesDelet, (err, result) => {
          ...
          if (!err) res.status(200).json({ isLike: false });
        });
      }
    });
  }
...
exports.getAllCommentsByID = (req, res, next) => {
  const sql =
    "SELECT comments.id,comments.id_user as userID, users.name as userName, users.last_name as userlastName, comments.comment FROM comments JOIN users on users.id = comments.id_user WHERE ?";
...
exports.verifAccesComment = (req, res, next) => {
  const sql = "SELECT id_user FROM comments WHERE ?";
  const value = { id: req.params.id };
  db.query(sql, value, (err, result) => {
    if (err) {
      console.log(err);
      ...
      result[0].id_user === parseInt(req.headers.authorization.split(" ")[2])
    ) {
      next();
    } else {
